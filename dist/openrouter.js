...
class OpenRouterService {
    constructor() {
        ...
        this.pageEmbeddingCache = new Map();
        this.websiteCache = new Map();
        const lcModel = new openai_2.ChatOpenAI({ model: "gpt-4o-mini", temperature: 0 });
        const embeddings = new openai_2.OpenAIEmbeddings();
        this.browser = new webbrowser_1.WebBrowser({ model: lcModel, embeddings });
    ...
    async precomputePageEmbeddings(allPages) {
    ...
    async fetchWebsiteLiveAnswer(query, urls) {
        // Return cached result if exists
        if (this.websiteCache.has(query))
            return this.websiteCache.get(query);
        // Embed the query
        ...
        const similarities = urls
            .map(url => ({
            url,
            sim: (0, utils_1.cosineSimilarity)(queryEmbedding, this.pageEmbeddingCache.get(url) || [])
        }))
            .sort((a, b) => b.sim - a.sim)
            .slice(0, 5); // top 5 relevant pages
        const limit = (0, p_limit_1.default)(3); // max 3 concurrent browser requests
        const tasks = similarities.map(({ url }) => limit(async () => {
            try {
        ...
        if (firstMatch)
            this.websiteCache.set(query, firstMatch);
        return firstMatch;
    }
    async generateReply(userMessage, userId) {
        ...
        const liveWebsiteContext = await this.fetchWebsiteLiveAnswer(userMessage, contant_1.allPages);
        let combinedContext = "";
        // if (blogContext) combinedContext += `\n\n📰 Blog context:\n${blogContext}`;
        if (pdfContext)
            combinedContext += `\n\n📖 PDF context:\n${pdfContext}`;
        if (liveWebsiteContext)
            combinedContext += `🌐 Live website context:\n${liveWebsiteContext}\n\n`;
